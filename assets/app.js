...
if (!customElements.get('collapsible-row')) {
  class CollapsibleRow extends HTMLElement {
    constructor() {
      ...
      // 新增属性：是否使用弹窗模式
      this.useModal = this.hasAttribute('use-modal') || false;
      // 初始化模态框元素
      this.modal = null;
            // 新增属性：点击跳转的链接
    ...
    initModal() {
      ...
      this.modal.className = 'accordion-modal';
    ...
    onClick(e) {
      e.preventDefault();
      if (this.link) {
        // 如果链接不是以http或https开始，则假设它是相对路径并转换成绝对路径
        const url = this.link.startsWith('http://') || this.link.startsWith('https://') ? this.link : new URL(this.link, window.location.origin).href;
        // 使用 window.open 并指定 '_blank' 来在新标签页打开链接
 ...
 */
if (!customElements.get('product-info-preview')) {
  class ProductInfoPreview extends HTMLElement {
    constructor() {
      super();
    }
    connectedCallback() {
    ...
    setupCCEventListener(e) {
      this.body.classList.remove('open-cc--product');
      this.drawer.classList.remove('product-info-preview-drawer');
      this.drawer.classList.remove('active');
    ...
    setupEventListener(e) {
      ...
      fetch(href, {
        ...
        .then(text => {
          ...
          const productSection = html.querySelector('section.product-section[id*="__main-product"], section.product-section');

          if (!productSection) {
            console.warn(`Product info preview: product section not found for ${productHandle}`);
            return;
          }

          const productDetail = productSection.querySelector('.thb-product-detail');
          const clonedProductSection = (productDetail || productSection).cloneNode(true);

          clonedProductSection.querySelectorAll('script[type="application/ld+json"], .side-panel, .pswp, product-add-to-cart-sticky').forEach((element) => {
            element.remove();
          });

          this.renderPreview(clonedProductSection.outerHTML, href, productHandle);

        });
    }
    renderPreview(sectionInnerHTML, href, productHandle) {
      if (sectionInnerHTML) {
        ...
        this.body.classList.add('open-cc--product');
        this.drawer.classList.add('product-info-preview-drawer');
        this.drawer.classList.add('active');
        ...
        dispatchCustomEvent('product-info-preview:open', {
          productUrl: href,
          productHandle: productHandle
        });
  ...
  customElements.define('product-info-preview', ProductInfoPreview);
}
